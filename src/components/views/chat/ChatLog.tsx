...
const ChatLog = (props: ChatLogType) => {
    ...
    if (isSender) {
      if (feedback.isSent && !feedback.isDelivered) {
        return (
          ...
          >
            {/* <Icon icon='tabler:close' fontSize='1.125rem' /> */}
  ...
  const renderChats = () => {
    return formattedChatData().map((item: FormattedChatsType, index: number) => {
      const isSender = item.senderId != data.chat.userId
      ...
      return (
        ...
        >
          ...
          <Box className='chat-body' sx={{ maxWidth: ['calc(100% - 5.75rem)', '75%', '65%'] }}>
            {item.messages.map(
              (chat: ChatLogChatType, index: number, { length }: { length: number }) => {
                      ...
                      <Typography variant='body2' sx={{ color: 'text.disabled' }}>
                        {time
                          ? new Date(time).getDate() === new Date().getDate()
                            ...
                            : new Date(time).toLocaleString('en-US', { month: 'numeric' }) +
                              '/' +
                              new Date(time).toLocaleString('en-US', { day: 'numeric' }) +
                              '/' +
